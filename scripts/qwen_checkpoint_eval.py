...
from scripts.llama_instruct_eval import SUITES, current_cases, _summary, _plot
...
def _signature(checkpoint: QwenCheckpoint, audit: dict, batch_size: int, environment: dict) -> dict:
    paths = (
    ...
    return {**_precision_identity(), **checkpoint.identity(), "tokenizer_audit": audit,
...
def run_qwen_eval(*, checkpoints: dict[str, QwenCheckpoint], local_root: Path, drive_root: Path,
                  ...
                  force_evaluation=False, persistence_kwargs=None) -> dict:
    ...
    if audit_tokenizer(tokenizer, current_cases()) != tokenizer_audit:
    ...
    for condition, checkpoint in checkpoints.items():
        signature = _signature(checkpoint, tokenizer_audit, batch_size, environment)
